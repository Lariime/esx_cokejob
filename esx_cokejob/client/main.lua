...
AddEventHandler('esx_cokejob:hasEnteredMarker', function(zone)
	...
	if zone == 'CokeFarm' then
		message = 'Ramasser Coke'
	elseif zone == 'CokeTreatment' then
		message = 'Conditionner Coke'
	elseif zone == 'Resell' then
		message = 'Vendre Pochons de Coke'
	end
...
AddEventHandler('esx_cokejob:hasExitedMarker', function(zone)
	...
	TriggerServerEvent('esx_cokejob:stopHarvestCoke')
	TriggerServerEvent('esx_cokejob:stopTransformCoke')
	...
	while true do
  	...
  	if GUI.ControlsShowed and IsControlPressed(0, Keys['ENTER']) and (GetGameTimer() - GUI.Time) > 1000 then
	  	...
	  	GUI.Time           = GetGameTimer()
			...
			if lastZone == 'CokeFarm' then
				TriggerServerEvent('esx_cokejob:startHarvestCoke')
			elseif lastZone == 'CokeTreatment' then
				TriggerServerEvent('esx_cokejob:startTransformCoke')
